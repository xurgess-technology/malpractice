...
- **The carry camera's crosshair is beside the head.** The aim ray follows the camera's line, so
  ...
  This used to bite any test that aims by head yaw and then reads `aim_prompt`, because the carry
  camera is active in ordinary play whenever the `camera` setting is "shoulder" or "front" -- and
  each slot seeds its settings from Zach's, which say "shoulder". It is what made four of
  `doortest`'s hinged-door E checks fail (2026-09-22; the door code was right), and it cost three
  other tasks time the same day. **You no longer have to do anything about it**: a headless run
  boots on `Settings.DEFAULTS`, where `camera` is `"first_person"`, so the carry camera is not
  active in a test unless the test turns it on (see `_machine_run` in `scripts/settings.gd`). The
  hand-rolled pins in `tools/doortest.gd` and `tools/devtest.gd` are gone. If your test *wants* the
  carry camera, set `camera` yourself -- and then aim the camera, as `tools/carrycamtest.gd` does,
  not the head.
