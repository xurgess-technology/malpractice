...
supply shelf's "SUPPLY - SURGICAL" tag): removed outright,
...
- `mark_db` is what `game._tick_scan` (sighted/scanned) and the harvest hook call. It is not
  cleared by `reset_money()` (a wipe): species knowledge is meant to
...
- A guest's scan or harvest is recorded exactly like the host's own: `_tick_scan` and the
  harvest hook are already host-only and iterate every player (`alive_players()`), so whichever
  peer is aiming or harvesting, the record it changes is `game.database` on the host.
...
else can read without digging through the layout (every space has some, since POCKET_SPACES_2 phase 5
gave the Factory and the Restaurant theirs). Every loot kind in it also needs `pocket` and `may_bleed` in
...
- **Per kind.** `LootTable.LOOT` gained `pocket` (which space a kind belongs to, the same set as the
  layout's `POCKET_ITEMS`) and `may_bleed`. Two kinds are `may_bleed: false` on purpose:
  `warm_scrubs`, a permanent cosmetics unlock that would devalue the Laundromat if it turned up in a
  corridor; and `restaurant_pagers`, the only bleed candidate with a `max_per_shift` — the cap is
  counted while the plan is made and the bleed swaps after it, so a bled station could be a shift's
  second set of bound pagers. The Chapel's `communion_wine` and the Restaurant's `tequila` are not
  loot at all (`Items.SURGICAL`, room-restricted in `ItemSpawner._legal`) and are untouched by this,
  as is the lone `restaurant_pager`, which has no `rooms` and no `pocket` and never spawns anywhere.
...
- `loot_table.gd`: `LOOT[kind]` `{name, short, value [min,max], tier 0..3, bulky, fragile, stack,
  ...
  eyes. Room keys are the real generated room kinds (`patient_room`, `supply_closet`,
...
No graft or graft stand yet (chunks B and C).
...
far: `icons`, `items`, `graft`, `graft_back`, `trinkets`, `puppet`, `gurney` (and many more: `SETUPS` is the list).
