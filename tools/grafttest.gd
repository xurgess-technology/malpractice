...
func _data_checks() -> void:
	...
	_check(tst.size() == 3 and tst[0].item == "scalpel" and tst[1].item == "scalpel" and tst[2].item == "forceps" and tst[2].variant == "place" and tst[0].site == "throat",
		"trachea extraction steps: scalpel, scalpel, forceps into the vat")
	var tgt: Array = Procedures.steps("trachea_graft")
	_check(tgt.size() == 4 and tgt[0].item == "scalpel" and tgt[1].item == "scalpel" and tgt[2].item == "forceps" and tgt[2].variant == "grab" and tgt[3].item == "suture_kit" and tgt[0].site == "throat",
		"trachea graft steps: scalpel, scalpel, forceps out of the vat, suture kit")
...
func _place_checks(script: GDScript) -> void:
	...
	var botches := [0]
	g.botched.connect(func(_a, _r): botches[0] += 1)
	g.finished.connect(func(r): in_vat[0] = bool(r.get("part_in_vat", false)))
	...
	while t < 40.0 and not in_vat[0]:
		...
		g.handle_cursor(inp.cursor, int(inp.buttons), dt)
		g.tick(dt)
	_check(in_vat[0] and botches[0] == 0, "the bot gets it into the vat: 'part_in_vat' after %.1f s, %d botches" % [t, botches[0]])
	g.free()
...
func _run() -> void:
	...
	_check(ok4 and String(c.state) == "stable" and bool(c.flags.get("part_in_vat", false)),
...
## Still strapped to table `ti`, with Botsworth operating and `vat` on the table.
func _trachea_checks(bw, ti: int, vat: Node) -> void:
	...
	bw.selected = _slot_of_for(bw, "scalpel")
	await _frames(3)
	_check(grafts.site_for(me) == "eye", "an eyeball vat on the table is an EYE graft, never a trachea one")
	...
	await _frames(3)
	_check(grafts.site_for(me) == "throat", "a trachea vat on the table is a THROAT graft")
	var offer := String(game._table_prompt(bw, ti))
