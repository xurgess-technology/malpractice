...
func _graft_checks() -> void:
	...
	_check(game.abilities.slot_of(me.peer_id, "puppet") >= 0 and game.abilities.level(me.peer_id, "hive") >= 1,
		"it gave Puppet 1 in an ability slot (slot %d, level %d)" % [game.abilities.slot_of(me.peer_id, "puppet"), game.abilities.level(me.peer_id, "hive")])
	...
	_check(game.abilities.slot_of(me.peer_id, "puppet") < 0, "and Puppet goes with it")
