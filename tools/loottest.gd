...
const BLEEDS := {
	...
	"factory": ["copper_wire_spool", "foremans_clipboard", "grease_bucket"],
	# restaurant_pagers is deliberately absent, and unlike warm_scrubs the reason is mechanical: it is
	# the only bleed candidate with a `max_per_shift`, and the bleed runs after the plan is made and
	# cannot honour one, so a bled station could be the shift's second. Tequila is absent because it
	# is a supply in Items.ITEMS, not loot -- the same reason the Chapel's communion wine is.
	"restaurant": ["cast_iron_molcajete"],
}
