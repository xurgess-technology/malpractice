# HIT FEEDBACK smoke look: runs tools/hitshot.tscn in a window that is minimized and never
# activated (the same WMI trick as review.ps1), so it can't take focus while Zach is working.
# Shots land in tools/hit_shots/, the log in .godot/hitshot.log.
param([string]$Extra = "")

. "$PSScriptRoot\godot_path.ps1"
$p = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$log = Join-Path $p ".godot\hitshot.log"
$a = @("--path", "`"$p`"", "--log-file", "`"$log`"", "--resolution", "1280x720",
	"res://tools/hitshot.tscn", "--", "--no-steam")
if ($Extra) { $a += "`"$Extra`"" }
...
Write-Host "hitshot pid $($r.ProcessId), log $log"
