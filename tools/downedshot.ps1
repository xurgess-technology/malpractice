# DOWNED smoke look: runs tools/downedshot.tscn in a window that is minimized and never activated
# (the same WMI trick as review.ps1), so it can't take focus while Zach is working.
# Shots land in tools/downed_shots/, the log in .godot/downedshot.log.
param([string]$Extra = "")

. "$PSScriptRoot\godot_path.ps1"
$p = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$log = Join-Path $p ".godot\downedshot.log"
$a = @("--path", "`"$p`"", "--log-file", "`"$log`"", "--resolution", "1280x720",
	"res://tools/downedshot.tscn", "--", "--no-steam")
if ($Extra) { $a += "`"$Extra`"" }
...
Write-Host "downedshot pid $($r.ProcessId), log $log"
