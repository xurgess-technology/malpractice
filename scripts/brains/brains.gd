extends Node
## Abilities. Per-player ability slots and levels, and the two abilities themselves: Echo
## (the sonographer path) and Hive Eyes (the hive path). A child "Brains" of Game on every machine
## -- the node keeps its old name; what it does is abilities, nothing else.
##
## Abilities come from grafts now (docs/GRAFTING.md, docs/GRAFTING_TRACHEA.md): a grafted Hive
## eyeball gives Hive Eyes, a grafted Sonographer trachea gives Echo. The grafting system calls
## set_level() / clear_ability(); nothing here earns a level on its own any more.
##
## Authority: the host decides everything (points, abilities, who is looking through which Hive).
## Clients get it through `net_state()` (global snapshot field `br`), the Player field `hive_view`
## (report key `hv`), and reliable events `br_echo`, `br_hive`. The visuals (the Echo view, the Hive
...
## slot_of() are independent of how a level is earned (grafting sources them, docs/GRAFTING.md), so
## slot code never reads `_points` directly except through level()/points().
...
## Host: game over. Every player's abilities are gone.
func on_reset() -> void:
...
## Every machine: a new level stops any Echo view still running from the old one.
...
func dev_request(sender: int, action: String, a: Dictionary) -> void:
	...
	match action:
		"br_levels":
			...
			game.say("Ability levels: Hive Eyes %d, Echo %d." % [level(id, "hive"), level(id, "sonographer")], 2.5)
		"br_reset":
			on_reset()
