...
## SWEEP 3 HOOK (abilities): looking through a Hive's eyes (Hive Eyes). Host authoritative, report
...
var bot_press: int = 0
## Bump to use the held item once (left mouse) / an ability once.
var bot_use: int = 0
...
func _input(event: InputEvent) -> void:
	...
	if hive_view or dev_input_held:
		return   # SWEEP 3 HOOK (abilities): the mouse is not yours while you look through a Hive
...
func _local_step(delta: float) -> void:
	...
	if can_move and bot_active:
		...
		if bot_press != _bot_press_seen:
			...
			if not diving:
				interact_count += 1
		# SWEEP 3 HOOK: scripted item use and abilities. HANDS HOOK: a use winds up first.
		...
		scan_holding = false
	# SWEEP 3 HOOK (abilities): Hive Eyes freezes the body; E or R asks to come back.
	...
	if can_move and not bot_active and not hive_view:   # SWEEP 3 HOOK (abilities): helpless in Hive Eyes
...
func _remote_step(delta: float) -> void:
	...
	head.rotation.x = lerpf(head.rotation.x, -0.95 if hive_view else _pitch, k)   # SWEEP 3 HOOK (abilities): head droops
	if _hive_glaze != null:
...
func _consume_actions() -> void:
	...
	# SWEEP 3 HOOK: item use and abilities (the systems decide what a busy player may do).
	...
	if interact_count != _interact_seen:
		...
		if hive_view or held_by >= 0:
			pass   # SWEEP 3 HOOK (abilities); held by the Nurse, nobody is coming in time
...
func _update_aim_core() -> void:
	...
	aim_prompt = ""
	aim_hold = 0.0
	if not alive or hive_view:   # SWEEP 3 HOOK (abilities): nothing in reach while you are elsewhere
		return
...
func revive_full() -> void:
	...
	hive_view = false   # SWEEP 3 HOOK (abilities)
...
func _update_down_pose(delta: float) -> void:
	...
	var tilt := -PI * 0.47 if down else (-0.2 if hive_view else echo_tilt)   # SWEEP 3 HOOK (abilities): slumped in Hive Eyes
...
func report_full() -> Dictionary:
	return {
		...
		"hv": hive_view,   # SWEEP 3 HOOK (abilities)
...
func apply_remote_full(s: Dictionary) -> void:
	...
	hive_view = bool(s.get("hv", false))   # SWEEP 3 HOOK (abilities)
