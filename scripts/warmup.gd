...
static func run(game: Node, progress: Callable = Callable(), ready_to_draw: Callable = Callable(), may_work: Callable = Callable()) -> void:
	...
	# dog_rig.gd). Warms its baked coat/vest grime-and-blood textures (Revision 13), the throat
	# orb's glass-shell and inner-wisp materials (the soul-drain sequence's harvestable piece,
	# dog_rig.gd's `_build_orb`, including its procedural swirl NoiseTexture2D) so the first Service
	# Dog of a session does not hitch on their texture generation or shader compile.
