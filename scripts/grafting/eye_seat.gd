...
##
##   grab  (Eyeball / Trachea Grafting, step 3)  out of the specimen vat on the table, into the empty
##                                      socket or throat. Finishes {"eye_seated": true}.
##   place (Eyeball / Trachea Extraction, last)  out of the socket or throat, into the vat.
##                                      Finishes {"part_in_vat": true}.
## On a throat (ctx.part_site "throat") what you carry is a length of windpipe instead of an eyeball.
## It lies along the throat, and it does not turn as it goes in.
##
...
var eye_r := 0.0135
## "eye" or "throat": the part on the plane is an eyeball or a length of windpipe, and the target is
## the socket or the open throat. The moves are the same.
var part_site := "eye"
...
func setup(owner, context: Dictionary, kind: String, radius: float) -> void:
	...
	eye_kind = kind
	eye_r = radius
	part_site = String(owner.get("part_site")) if owner != null and owner.get("part_site") != null else "eye"
	...
	match stage:
		...
		Stage.HELD:
			...
			if not primary:
				...
				if tip.distance_to(target_at()) <= drop_reach():
					...
					drops += 1
					stage = Stage.SOURCE
					_hint = "It dropped back in the %s. Pick it up again." % ("vat" if mode == "grab" else _site_word())
					_hint_t = 3.0
		Stage.SEATING:
		...
		Stage.SETTLE:
			...
			if settle >= SETTLE_TIME:
				stage = Stage.DONE
				_update_progress()
				owner_mg.finish({"eye_seated": true} if mode == "grab" else {"part_in_vat": true})
				return
...
func hud_state() -> Dictionary:
	...
	var to_where := _target_word()
	if hint == "":
		match stage:
			Stage.SOURCE:
				hint = "Hold left click on the %s, drag it to the %s, and let go." % [_part_word(), to_where]
			Stage.HELD:
...
## The words for what is carried, where it is going, and the site it goes to or from.
func _part_word() -> String:
	return "windpipe" if part_site == "throat" else "eye"


func _site_word() -> String:
	return "throat" if part_site == "throat" else "socket"


func _target_word() -> String:
	return _site_word() if mode == "grab" else "vat"
...
func keys() -> Array:
	...
	match stage:
		Stage.SOURCE:
			return [["Hold LMB", "grab the " + _part_word()], ["Mouse", "drag it to the " + to_where]]
		Stage.HELD:
...
func _build() -> void:
	...
	add_child(_eye_node)
	if part_site == "throat":
		Eyes.as_trachea(_eye_node, eye_kind, eye_r)
...
func tick(delta: float) -> void:
	...
	if part_site == "throat":
		turn = 0.0   # a windpipe lies flat along the throat; there is no pupil to face out
	...
	if part_site == "throat":
		_nerve.visible = false   # a windpipe is not on a nerve: it is just lifted
	elif held:
